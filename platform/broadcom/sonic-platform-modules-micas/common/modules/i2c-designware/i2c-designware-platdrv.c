...
static int sda_fall_ns = 0;
module_param(sda_fall_ns, int, S_IRUGO);
static int scl_fall_ns = 0;
module_param(scl_fall_ns, int, S_IRUGO);
static int bus_freq_hz = 0;
module_param(bus_freq_hz, int, S_IRUGO);
...
#if 0//def CONFIG_I2C_DESIGNWARE_BAYTRAIL
	{
...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	/* Override default fall timeouts if specified by platform data or device tree */
	if (sda_fall_ns || scl_fall_ns) {
		struct i2c_timings *t = &dev->timings;
		t->sda_fall_ns = sda_fall_ns;
		t->scl_fall_ns = scl_fall_ns;
	}
